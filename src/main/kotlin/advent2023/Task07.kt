...
object Task07 : Task {
    ...
    override fun partB(): Int {
        ...
        parseInput().forEach { (hand, bid) ->
            val handsWithJoker = cardOrderWithJoker.map { hand.replace("J", it) }
            when {
        ...
        distinct.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        pairs.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        twoPairs.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        threeOfAKinds.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        fullHouses.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        fourOfAKinds.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
        fiveOfAKinds.sortCards(cardOrderWithJoker).forEach { allCardsOrdered.add(it) }
